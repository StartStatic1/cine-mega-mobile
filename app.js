...
// Slider 9:16 Automático e Manual
    ...
    wrapper.innerHTML = movies.map((m, i) => `
        <div class="hero-item" style="background-image: url(https://image.tmdb.org/t/p/original${m.poster_path})">
            <div class="hero-overlay">
                ...
                <p>${m.overview}</p>
                <button onclick="abrir(${m.id})" style="background:#fff; color:#000; border:none; padding:12px; border-radius:8px; font-weight:bold; width:140px;">▶ ASSISTIR</button>
            </div>
        </div>
    `).join('');
...
// Seções de Curadoria (Trash, Estreias, Clássicos)
...
function abrirVLC() {
    ...
    window.location.href = `intent://${url.replace(/^https?:\/\//, '')}#Intent;scheme=http;package=org.videolan.vlc;S.title=${filmeAtual};end`;
}

function abrirMX() {
    const url = `${MOTOR}/buscar?titulo=${encodeURIComponent(filmeAtual)}`;
    window.location.href = `intent://${url.replace(/^https?:\/\//, '')}#Intent;scheme=http;package=com.mxtech.videoplayer.ad;S.title=${filmeAtual};end`;
}
