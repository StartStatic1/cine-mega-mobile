...
// 1. SISTEMA DE LOGIN (VALIDAÇÃO LOCAL PARA NÃO DAR ERRO DE CONEXÃO)
function validarChave() {
    const input = document.getElementById('inputChave').value.trim();
    // Lista de chaves manuais para garantir acesso imediato
    const chaves = ["0980", "0981", "0982", "0983"];
    
    if(chaves.includes(input)) {
...
// 2. CARREGAR HOME (MANTENDO SEU LAYOUT ORIGINAL)
...
async function carregarHome() {
    ...
    // EM BREVE (Ajustado para não bugar o layout)
...
// 3. DETALHES E VACINA DO & (INTERLIGADO COM MOTOR/ARCHIVE)
async function abrir(id, isEmBreve = false) {
    ...
    // Título limpo para o Sniper/Archive.org
    ...
    if(isEmBreve) {
        // BLOQUEIO APENAS DOS PLAYERS
        btnPlay.style.background = "#222"; btnPlay.style.color = "#444";
        btnPlay.innerText = "BLOQUEADO"; btnPlay.onclick = null;
        boxPlayers.innerHTML = `<div style="width:50px; height:50px; border-radius:50%; background:#111; display:flex; align-items:center; justify-content:center; color:#333; font-weight:bold; border:1px solid #222;">VLC</div><div style="width:50px; height:50px; border-radius:50%; background:#111; display:flex; align-items:center; justify-content:center; color:#333; font-weight:bold; border:1px solid #222;">MX</div>`;
...
document.addEventListener("DOMContentLoaded", () => {
    if(localStorage.getItem('cine_mega_acesso') === 'LIBERADO') liberarApp();
});
